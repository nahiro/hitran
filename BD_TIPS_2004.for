...
C*********************** 
C    Subroutine BD_TIPS_2004 written by R.R. Gamache
C 
c
c...date last changed 18     December, 2003
c  --  UPDATES  --
...
c++
      PARAMETER (NT=119)
c++
      COMMON /ISO_data_2004/ISO82(NMOL,Max_ISO),ISONM(NMOL)
c++:  bd-MOL
      CHARACTER*6 MOLID
c++:  bd-MOL
      COMMON/MOLNAM_2004/MOLID(0:NMOL)
c++:  bd-QT
      COMMON/Temperatures_2004/tdat(NT)
...
C     GO TO PARTICULAR MOLECULE:  
      ...
      IF(MOL.EQ.1) THEN 
      CALL QT_H2O_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.2) THEN 
      CALL QT_CO2_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.3) THEN 
      CALL QT_O3_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.4) THEN 
      CALL QT_N2O_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.5) THEN 
      CALL QT_CO_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.6) THEN 
      CALL QT_CH4_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.7) THEN 
      CALL QT_O2_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.8) THEN 
      CALL QT_NO_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.9) THEN 
      CALL QT_SO2_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.10) THEN
      CALL QT_NO2_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.11) THEN
      CALL QT_NH3_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.12) THEN
      CALL QT_HNO3_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.13) THEN
      CALL QT_OH_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.14) THEN
      CALL QT_HF_2004(Temp,ISO,gi,QT) 
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.15) THEN
      CALL QT_HCL_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.16) THEN
      CALL QT_HBR_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.17) THEN
      CALL QT_HI_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.18) THEN
      CALL QT_CLO_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.19) THEN
      CALL QT_OCS_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.20) THEN
      CALL QT_H2CO_2004(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.21) THEN
      CALL QT_HOCL_2004(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.22) THEN
      CALL QT_N2_2004(Temp,ISO,gi,QT) 
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.23) THEN
      CALL QT_HCN_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.24) THEN
      CALL QT_CH3CL_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.25) THEN
      CALL QT_H2O2_2004(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.26) THEN
      CALL QT_C2H2_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.27) THEN
      CALL QT_C2H6_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.28) THEN
      CALL QT_PH3_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.29) THEN 
      CALL QT_COF2_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.30) THEN 
      CALL QT_SF6_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.31) THEN
      CALL QT_H2S_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.32) THEN
      CALL QT_HCOOH_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.33) THEN 
      CALL QT_HO2_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.34) THEN 
...
c
      IF(MOL.EQ.35) THEN 
      CALL QT_ClONO2_2004(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.36) THEN 
      CALL QT_NOp_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
      IF(MOL.EQ.37) THEN 
      CALL QT_HOBr_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
      IF(MOL.EQ.38) THEN 
      CALL QT_C2H4_2004(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
 100  RETURN
...
C
   ...
   10 CONTINUE
      ...
      include 'BD_MOL_2004.for'
      include 'BD_ISO_2004.for'
  
  
c
c     *****************
      Subroutine QT_H2O_2004   (                       
...
c...value depends on temperature range
      ...
      call AtoB_2004(T,Qt,Tdat,Q,NT)
c      
   99 return
      end
c
c     *****************
      Subroutine QT_CO2_2004   (                       
     I T, 	! temperature in K 
     I iso, 	! isotope code (HITRAN INDEX)
     O gsi, 	! state independent nuclear degeneracyfactor
     O QT) 	! Total Internal Partition Function
...
c
c***************************
      SUBROUTINE AtoB_2004(aa,bb,A,B,npt)
c***************************
