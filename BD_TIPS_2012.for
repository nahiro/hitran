...
C*********************** 
C    Subroutine BD_TIPS_2012 written by R.R. Gamache
C 
c
...
c..  
c..  (d)  Laboratoire de Photophysique Mol?culaire, Universit? Paris Sud, 91405 Orsay, FRANCE
c..  
...
c++:  bd-QT
      COMMON/Temperatures_2012/tdat(NT)
...
C     GO TO PARTICULAR MOLECULE:  
      ...
      IF(MOL.EQ.1) THEN 
      CALL QT_H2O_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.2) THEN 
      CALL QT_CO2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.3) THEN 
      CALL QT_O3_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.4) THEN 
      CALL QT_N2O_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.5) THEN 
      CALL QT_CO_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.6) THEN 
      CALL QT_CH4_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.7) THEN 
      CALL QT_O2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.8) THEN 
      CALL QT_NO_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.9) THEN 
      CALL QT_SO2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.10) THEN
      CALL QT_NO2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.11) THEN
      CALL QT_NH3_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.12) THEN
      CALL QT_HNO3_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.13) THEN
      CALL QT_OH_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.14) THEN
      CALL QT_HF_2012(Temp,ISO,gi,QT) 
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.15) THEN
      CALL QT_HCL_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.16) THEN
      CALL QT_HBR_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.17) THEN
      CALL QT_HI_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.18) THEN
      CALL QT_CLO_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.19) THEN
      CALL QT_OCS_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.20) THEN
      CALL QT_H2CO_2012(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.21) THEN
      CALL QT_HOCL_2012(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.22) THEN
      CALL QT_N2_2012(Temp,ISO,gi,QT) 
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.23) THEN
      CALL QT_HCN_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
C 
      IF(MOL.EQ.24) THEN
      CALL QT_CH3CL_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.25) THEN
      CALL QT_H2O2_2012(Temp,ISO,gi,QT) 
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.26) THEN
      CALL QT_C2H2_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.27) THEN
      CALL QT_C2H6_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
C 
      IF(MOL.EQ.28) THEN
      CALL QT_PH3_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.29) THEN 
      CALL QT_COF2_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.30) THEN 
      CALL QT_SF6_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.31) THEN
      CALL QT_H2S_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.32) THEN
      CALL QT_HCOOH_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.33) THEN 
      CALL QT_HO2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.34) THEN 
...
c
      IF(MOL.EQ.35) THEN 
      CALL QT_ClONO2_2012(Temp,ISO,gi,QT)
      go to 100 
      ENDIF 
c
      IF(MOL.EQ.36) THEN 
      CALL QT_NOp_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
      IF(MOL.EQ.37) THEN 
      CALL QT_HOBr_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
      IF(MOL.EQ.38) THEN 
      CALL QT_C2H4_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
      IF(MOL.EQ.39) THEN 
...
c
      IF(MOL.EQ.40) THEN 
      CALL QT_CH3Br_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.41) THEN 
      CALL QT_CH3CN_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF 
c
      IF(MOL.EQ.42) THEN 
      CALL QT_CF4_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.43) THEN 
      CALL QT_C4H2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.44) THEN 
      CALL QT_HC3N_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.45) THEN 
      CALL QT_C2N2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.46) THEN 
      CALL QT_CS_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.47) THEN 
      CALL QT_H2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.48) THEN 
      CALL QT_SO_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.49) THEN 
      CALL QT_C3H4_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.50) THEN 
      CALL QT_CH3_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
c
      IF(MOL.EQ.51) THEN 
      CALL QT_CS2_2012(Temp,ISO,gi,QT)
      go to 100
      ENDIF     
C 
 100  RETURN
...
C
   ...
   10 CONTINUE
      ...
      include 'BD_MOL_2012.for'
      include 'BD_ISO_2012.for'
  
  
c
c     *****************
      Subroutine QT_H2O_2012   (                       
...
c...value depends on temperature range
      ...
      call AtoB_2012(T,Qt,Tdat,Q,NT)
c      
   99 return
      end
c
c     *****************
      Subroutine QT_CO2_2012   (                       
     I T, 	! temperature in K 
     I iso, 	! isotope code (HITRAN INDEX)
     O gsi, 	! state independent nuclear degeneracyfactor
     O QT) 	! Total Internal Partition Function
...
c
c***************************
      SUBROUTINE AtoB_2012(aa,bb,A,B,npt)
c***************************
